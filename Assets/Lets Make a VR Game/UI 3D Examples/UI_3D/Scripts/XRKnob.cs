...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRKnob : XRBaseInteractable
    {
        ...
        [SerializeField]
        [Tooltip("The object that will appear gradually as the knob is rotated")]
        private GameObject objectToAppear; // الأوبجكت الذي سيظهر
        [SerializeField]
        ...
        [SerializeField]
        private float heightToShowText = 3.0f; // الارتفاع الذي عنده يظهر النص


        [SerializeField]
        private AudioSource soundSourceObject1; // مصدر الصوت للأوبجكت الأول
        [SerializeField]
        private AudioSource soundSourceObject2; // مصدر الصوت للأوبجكت الثاني
        ...
        {
            base.Awake();
            m_OnValueChange.AddListener(UpdateObjectAppearance);
            if (objectToAppear != null)
                objectToAppear.SetActive(false); // إخفاء الأوبجكت في البداية
        }
        ...
        private void UpdateObjectAppearance(float newValue)
        {
            if (objectToAppear != null)
            {
                objectToAppear.SetActive(newValue > 0);

                if (objectToAppear.activeSelf)
                {
                    var renderer = objectToAppear.GetComponent<Renderer>();
                    if (renderer != null)
                    {
                    ...
                    // تشغيل الصوت للأوبجكت الأول
                    if (newValue > 0 && !soundSourceObject1.isPlaying)
                    {
                        soundSourceObject1.Play();
                    }
                    Vector3 newPosition = objectToAppear.transform.position;
                    newPosition.y = Mathf.Lerp(minY, maxY, newValue);
                    objectToAppear.transform.position = newPosition;

                    // تفعيل أو إخفاء GameObject بناءً على الارتفاع
                    if (newPosition.y >= heightToShowText)
                    {
                        // تشغيل الصوت للأوبجكت الثاني
                        if (!soundSourceObject2.isPlaying)
                        {
                            soundSourceObject2.Play();
                        }

                        objectToShow.SetActive(true); // إظهار الأوبجكت

                    }
                    else
                    {
                        objectToShow.SetActive(false); // إخفاء الأوبجكت
                    }
                }
            }
        }
